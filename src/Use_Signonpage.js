...
const AuthPage = ({ isLogin = true }) => {
  ...
  const login = async () => {
    ...
    try {
      ...
      if (result.status === 200 && result.user) {
        ...
        const subscriptionPlanId = result.user.subscription_plan_id || "";
        const userEmail = result.user.email || "";

        setStoredPlanId(subscriptionPlanId);
        ...
        // Check category setup after successful login
        setTimeout(() => {
        verifyCategorySetup();
        }, 100);
      } else {
  ...
  // Verify category setup function
  const verifyCategorySetup = async () => {
    try {
  ...
  const authenticateWithBackend = async (accessToken, isLoginMode) => {
    try {
